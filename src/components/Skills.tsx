...
const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-16 md:py-20 lg:py-24 bg-dark relative overflow-hidden">
      {/* Background elements */}
      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 px-4 sm:px-6">
          {skillCategories.map((category, index) => (
            ...
            >
              ...
              <div className="p-4 sm:p-6 lg:p-8">
                <h3 className="text-lg md:text-xl font-display font-semibold text-light mb-4 lg:mb-6 flex items-center gap-2">
                  <span className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-accent/20 flex items-center justify-center text-accent text-sm md:text-base group-hover:bg-accent group-hover:text-light transition-colors duration-300">
                    {index + 1}
                  </span>
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {category.skills.map((skill, skillIndex) => (
                    <span 
                      key={skillIndex} 
                      className="px-3 py-1 md:px-4 md:py-1.5 bg-surface/50 text-light/80 text-xs sm:text-sm rounded-full border border-accent/10 hover:border-accent/30 hover:bg-surface/80 transition-all duration-300"
                    >
                      {skill}
                    </span>
                  ))}
        ...
        <div className="mt-12 md:mt-16 lg:mt-20 text-center px-4">
          <p className="text-light/70 mb-6 md:mb-8 max-w-2xl mx-auto">
